<template>
    ...
    <div class="container-fluid">
        ...
        <div v-if="c_c_p_p_orders.length" class="row mt-5">

            <CCP v-on:file-delete-success="file_delete_success" v-for="(c_c_p_p_order, index) in c_c_p_p_orders" :key="index" :c_c_p_p_order="c_c_p_p_order" />

        </div>

        <div v-if="c_c_p_p_orders.length <= 0" class="row pt-5">
...
export default {
    ...
    mounted() {
        ...
        .then( (resp) => {
            ...
            console.log("CCCCCC", this.stage)
            
            if(this.stage.produce_center_order !== null) this.produce_center_order = this.stage.produce_center_order

            if(this.stage.c_c_p_p_orders.length) this.c_c_p_p_orders = this.stage.c_c_p_p_orders
    ...
    methods: {
        ...
        file_delete_success(data) {
            ...
            for (const c_c_p_p_orders of this.c_c_p_p_orders) {
                
                if(c_c_p_p_orders.id !== data.id) new_items.push(c_c_p_p_orders)

            }

            this.c_c_p_p_orders = new_items

            // this.c_c_p_p_orders = this.c_c_p_p_orders.filter( (c_c_p_p_orders) => { c_c_p_p_orders.id !== data.id } )
