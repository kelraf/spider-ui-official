    ...
    watch: {
        stageData: {
            ...
            handler() {
                ...
                if(Object.keys(this.stageData).length > 0) this.options.params.livestock_order_stage_id = this.stageData.id
        ...
        handleUploadError: function(file, message, xhr) {
            ...
            if(xhr.status == 422) {
                ...
                for (const key of Object.keys(JSON.parse(xhr.response).errors)) {
                    ...
                    if(key == "purpose") {
                        this.$toasted.show(`${JSON.parse(xhr.response).errors.purpose[0]}`, {theme: 'outline',position: "top-right", icon : 'times', type: 'error', duration: 3000})
                    } else if(key == "file") {
                        this.$toasted.show(`${JSON.parse(xhr.response).errors.file[0]}`, {theme: 'outline',position: "top-right", icon : 'times', type: 'error', duration: 3000})
                    } 
                }
