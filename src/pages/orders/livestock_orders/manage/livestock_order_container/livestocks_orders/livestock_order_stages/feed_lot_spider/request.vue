    ...
    <div class="container-fluid">
        <div @click.self="close" class="row">
            <div class="col-md-6 offset-md-3">
                ...
                <form-wizard color="#4466f2" class="bg-white p-4 m-2" :start-index="0"  title="Feed Lot Request Wizard" subtitle="Go Through The Wizard To Complete Request Info" back-button-text="Go back!" next-button-text="Go next!" finish-button-text="We're there">
                                                ...
                                                <tbody slot="body" slot-scope="{displayData}">
                                                    <tr v-for="row in displayData" :key="row.id">
                                                        ...
                                                        <td>
                                                            ...
                                                            <button :disabled="Object.keys(feed_lot_livestock_order).length > 0 ? true : false " v-if="!row.selected" @click="selectIt(row)" id="default-outline-primary" type="button" class="btn btn-pill btn-outline-primary btn-block">
                                                                Select
                                                            </button>
...
export default {
    ...
    watch: {
        stageData: {
            ...
            handler() {
                ...
                if(Object.keys(this.stageData).length > 0) {
                    ...
                    if(this.stage_data.feed_lot_livestock_order !== null) {
                        this.feed_lot_livestock_order = this.stage_data.feed_lot_livestock_order
                        this.dates.starting_date = new Date(this.feed_lot_livestock_order.dates.starting_date)
                        this.dates.ending_date = new Date(this.feed_lot_livestock_order.dates.ending_date)
                        // this.editorData = this.feed_lot_livestock_order.description
                    }
        ...
        feedLots: {
            ...
            handler() {
                ...
                if(this.feedLots.length > 0) this.feed_lots = this.feedLots

                if(this.feedLots.length > 0) this.feed_lots.map( (feed_lot) => { if(feed_lot.selected) this.selected_feed_lot = feed_lot } )
    ...
    methods: {
        ...
        checkSelected() {
            ...
            if(Object.keys(this.selected_feed_lot).length == 0) {

                this.$toasted.show(`Please Select A FeedLot.`, {theme: 'outline',position: "top-right", icon : 'times', type: 'error', duration: 4000})
                return false
        ...
        sendRequest() {
            ...
            if(this.stage_data.feed_lot_livestock_order == null) {
                ...
                axios.post(`${ApiUrl.url}feed-lot-livestock-orders`, data, headers) 
                .then( (resp) => {
                    setTimeout(function() {

                        self.loading = false
                        self.$emit("feed-lot-request-created-success", resp.data.data)
                ...
                .then( (resp) => {
                    setTimeout(function() {
                        ...
                        self.loading = false
                        self.$emit("feed-lot-request-updated-success", resp.data.data)
