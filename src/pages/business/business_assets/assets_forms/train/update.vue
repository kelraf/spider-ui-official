                        ...
                        <div class="authentication-box">
                            <div class="card to-shake mt-4 p-4">
                                <form class="theme-form">
                                    <h5 class="f-16 mb-3 f-w-600"> Add Train </h5>
    ...
    watch: {
        trainData: {
            ...
            handler() {
                ...
                this.form.unique_number.value = this.train_data.unique_number != null ? this.train_data.unique_number: ''
                this.form.role.value = this.train_data.role != null ? this.train_data.role: ''
            }
        }
    },
    methods: {
            ...
            if(this.form.unique_number.value == '') {
                ...
                let data = {
                    train : {
                        ...
                        user_id: this.train_data.user_id,
                        business_id: this.train_data.business_id
                    }
                }                
                ...
                axios.put(`${ApiUrl.url}trains/${this.train_data.id}`, data, headers)
                .then( (resp) => {
                    setTimeout(function() {

                        self.loading = false
                        self.$emit("train-updated-success", resp.data.data)
                        self.$toasted.show(`Train Created Successfully`, {theme: 'outline', position: "top-right", icon : 'check', type: 'success', duration: 8000})
                        Custombox.modal.close()
