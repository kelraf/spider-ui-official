                        ...
                        <div class="authentication-box">
                            <div class="card to-shake mt-4 p-4">
                                <form class="theme-form">
                                    <h5 class="f-16 mb-3 f-w-600"> Update Product </h5>
    ...
    watch: {
        productData: {
            ...
            handler() {
                ...
                this.form.product_name.value = this.productData.name == null ? '' : this.productData.name
                this.form.type.value = this.productData.type == null ? '' : this.productData.type
                this.form.category.value = this.productData.category == null ? '' : this.productData.category
                this.form.quantity.value = this.productData.quantity == null ? '' : this.productData.quantity
                this.form.units.value = this.productData.units == null ? '' : this.productData.units
                this.form.price.value = this.productData.price == null ? '' : this.productData.price
                this.form.business_id = this.productData.business_id == null ? '' : this.productData.business_id
                this.form.user_id = this.productData.user_id == null ? '' : this.productData.user_id,
                this.form.id = this.productData.id
    ...
    methods: {
        ...
        create: function() {
            ...
            if(this.form.product_name.value == '') {
                ...
                axios.put(`${ApiUrl.url}products/${this.form.id}`, data, headers) 
                .then( (resp) => {
                    setTimeout(function() {

                        self.loading = false
                        self.$emit("product-update-success", resp.data.data)
                        self.$toasted.show(`Product Updated Successfully`, {theme: 'outline',position: "top-right", icon : 'check', type: 'success', duration: 8000})
                        Custombox.modal.close()
