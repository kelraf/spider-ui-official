    ...
    methods: {
        ...
        create: function() {
            ...
            if(this.form.registration_number.value == '') {
                ...
                axios.post(`${ApiUrl.url}vehicles`, data, headers) 
                .then( (resp) => {
                    setTimeout(function() {

                        self.loading = false
                        self.$emit("vehicle-create-success", resp.data.data)
                        self.$toasted.show(`Vehicle Created Successfully`, {theme: 'outline',position: "top-right", icon : 'check', type: 'success', duration: 8000})
                        Custombox.modal.close()
