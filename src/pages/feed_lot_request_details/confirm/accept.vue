...
export default {
    ...
    methods: {
        ...
        go() {
            ...
            axios.put(`${ApiUrl.url}feed-lot-livestock-orders/${this.action_data.id}`, data, headers) 
            .then( (resp) => {

                setTimeout(function() {

                    self.loading = false
                    self.$emit("feed-lot-request-edit-success", resp.data.data)
                    self.$toasted.show(`Feed Lot Request Accepted.`, {theme: 'outline', position: "top-right", icon : 'check', type: 'success', duration: 8000})
                    Custombox.modal.close()
